...
def convert_elementwise_div(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if is_numpy(layers[node.input[0]]) and is_numpy(layers[node.input[1]]):
        ...
        if(type(input_0) == np.ndarray):
            try:
                temp = input_0[0]
            except:
                input_0 = np.array([input_0])
                
        if(type(input_1) == np.ndarray):
            try:
                temp = input_1[0]
            except:
...
def convert_elementwise_add(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if(input_0.shape == ()):
        logger.warning(f'Temp Fix')
        input_0 = np.array([1], dtype=np.float32)
        
    if(input_1.shape == ()):
        logger.warning(f'Temp Fix')
        input_1 = np.array([1], dtype=np.float32)
...
def convert_elementwise_mul(node, params, layers, lambda_func, node_name, keras_name):
    ...
    try:
        if(input_1.shape[0] == None):
            input_1 = tf.squeeze(input_1, [0])
            
        if len(input_0.shape) == 1 and (len(input_0.shape) < len(input_1.shape)):
            input_0 = tf.reshape(input_0, [1] * (len(input_1.shape) - 1) + [-1])
            print(input_0)
    ...
    print(f"output: {layers[node_name].shape} \n\n") 
