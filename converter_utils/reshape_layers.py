        ...
        if len(layer_input) > 1:
            try:
                ...
                for i in range(len(layer_input)):
                    if (layer_input[i].dtype == "int32"):
                        layer_input[i] = tf.cast(layer_input[i]._inferred_value, dtype=tf.int64)
...
def convert_slice(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if is_numpy(layers[node.input[0]]):
        ...
        # else:
            ...
            #     print(f"{i}: {layers[node.input[i]]}")
        #     raise AttributeError('Not implemented')
                        
        starts = layers[node.input[1]].item(0)
        ends = layers[node.input[2]].item(0)
        axes = layers[node.input[3]].item(0)
